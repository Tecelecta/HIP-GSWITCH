...
struct device_graph_t<COO, E>{
  ...
  int64_t build(host_graph_t<COO,E> hgraph){
    ...
    int64_t gpu_bytes=0;
    H_ERR(hipMalloc((void**)&dg_coo, sizeof(packed_t)*hgraph.nedges));
    H_ERR(hipMemcpy(dg_coo,  hgraph.edge_list,  sizeof(packed_t)*hgraph.nedges, H2D));
  ...
  int level;
  packed_t* dg_coo;
  hipTextureObject_t dt_edgelist;
};
...
struct device_graph_t<CSR, E>{
  ...
  int64_t build(host_graph_t<CSR,E> hgraph){
    ...
    int64_t gpu_bytes=0;
    H_ERR(hipMalloc((void**)&dg_adj_list,  sizeof(int)*hgraph.nedges)); 
    H_ERR(hipMalloc((void**)&dg_odegree,   sizeof(int)*hgraph.nvertexs));
    H_ERR(hipMalloc((void**)&dg_start_pos, sizeof(int)*hgraph.nvertexs));

    H_ERR(hipMemcpy(dg_adj_list,  hgraph.adj_list,  sizeof(int)*hgraph.nedges, H2D));
    H_ERR(hipMemcpy(dg_odegree,   hgraph.odegrees,  sizeof(int)*hgraph.nvertexs, H2D));
    H_ERR(hipMemcpy(dg_start_pos, hgraph.start_pos, sizeof(int)*hgraph.nvertexs, H2D));

    if(directed){
      H_ERR(hipMalloc((void**)&dgr_adj_list,  sizeof(int)*hgraph.nedges)); 
      H_ERR(hipMalloc((void**)&dgr_odegree,   sizeof(int)*hgraph.nvertexs));
      H_ERR(hipMalloc((void**)&dgr_start_pos, sizeof(int)*hgraph.nvertexs));

      H_ERR(hipMemcpy(dgr_adj_list,  hgraph.r_adj_list,  sizeof(int)*hgraph.nedges, H2D));
      H_ERR(hipMemcpy(dgr_odegree,   hgraph.r_odegrees,  sizeof(int)*hgraph.nvertexs, H2D));
      H_ERR(hipMemcpy(dgr_start_pos, hgraph.r_start_pos, sizeof(int)*hgraph.nvertexs, H2D));
    }

    if(!std::is_same<Empty,E>::value){
      LOG(" -- edgedata is not Empty\n");
      H_ERR(hipMalloc((void**)&dg_edgedata,  sizeof(E)*hgraph.nedges)); 
      H_ERR(hipMemcpy(dg_edgedata, hgraph.edge_weights, sizeof(int)*hgraph.nedges, H2D));

      if(directed){
        H_ERR(hipMalloc((void**)&dgr_edgedata,  sizeof(E)*hgraph.nedges)); 
        H_ERR(hipMemcpy(dgr_edgedata, hgraph.r_edge_weights, sizeof(int)*hgraph.nedges, H2D));
      }
