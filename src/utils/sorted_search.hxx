...
#define TILE_SZ (THD_NUM<<1)
  auto partition_kernel = partition<data_t, TILE_SZ>;
  auto block_upper_kernel = block_upper<data_t, TILE_SZ>;
  ...
  hipLaunchKernelGGL(partition_kernel, dim3(1 + CEIL(tile_count, THD_NUM)), dim3(THD_NUM), 0, 0, dg_a, a_count, dg_b, b_count, tile_count, dg_atile, dg_btile);
...
//  dump_arr(dg_btile, tile_count);
  hipLaunchKernelGGL(partition_kernel, dim3(tile_count-1), dim3(THD_NUM), 0, 0, dg_a, a_count, dg_b, b_count, dg_atile, dg_btile, dg_idx);
#undef TILE_SZ
}
