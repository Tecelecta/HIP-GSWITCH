...
__host__ void __filter_atomic(active_set_t& as, config_t conf){
  ...
  if(conf.conf_target == Active) { 
    hipLaunchKernelGGL(__filter_active<M>, dim3(ctanum), dim3(thdnum), 0, 0, as); 
  } else if(conf.conf_target == Inactive) { 
    hipLaunchKernelGGL(__filter_inactive<M>, dim3(ctanum), dim3(thdnum), 0, 0, as);
  }else{ 
    //__copy_all<M><<<ctanum,thdnum>>>(as); 
    hipLaunchKernelGGL(__filter_unfixed<M>, dim3(ctanum), dim3(thdnum), 0, 0, as);
  } 
}
...
__host__ void __filter_scan(active_set_t &as, config_t conf){ 
  if(conf.conf_cm == Interleave) 
    hipLaunchKernelGGL(__filter_local_stride, dim3(CTANUM), dim3(THDNUM), 0, 0, as, conf); 
  else
    hipLaunchKernelGGL(__filter_local_interleave, dim3(CTANUM), dim3(THDNUM), 0, 0, as, conf); 
  ...
  hipLaunchKernelGGL(__compress, dim3(CTANUM), dim3(THDNUM), 0, 0, as.bins.dg_bin, as.bins.dg_size,
